...
cxxopts::Options create_commandline_parser(const char *argv[]) {
  cxxopts::Options options(argv[0],
                           "Echoframe beamformer A matrix preparation");

  options.add_options()("a_matrix_in", "Full path to input A matrix",
                        cxxopts::value<std::string>())(
      "a_matrix_out", "Full path to output A matrix",
      cxxopts::value<std::string>())("pixels", "Number of pixels",
...
cxxopts::ParseResult parse_commandline(int argc, const char *argv[]) {
  ...
  try {
    ...
    std::vector<std::string> required_options{"a_matrix_in", "a_matrix_out",
                                              "pixels", "samples"};
...
int main(int argc, const char *argv[]) {
  ...
  const std::string path_a_matrix_out =
      cmdline["a_matrix_out"].as<std::string>();
  ...
  const size_t bytes_a_matrix_packed =
      2UL * pixels_padded * samples_padded / CHAR_BIT;
  ...
  for (size_t c = 0; c < 2; c++) {
    ...
    const size_t complex_offset_device_packed =
        c * pixels_padded * samples_padded / CHAR_BIT;
    // process chunks
    for (size_t pixel_start = 0; pixel_start < pixels;
         pixel_start += pixels_per_chunk) {
      ...
      for (size_t pixel = 0; pixel < local_npixels; pixel++) {
        ...
        const size_t offset =
            (pixel_start + pixel) * samples + complex_offset_host;
        const size_t bytes_to_transfer = samples;

        cu::DeviceMemory d_a_chunk_slice(d_a_chunk, d_offset,
                                         bytes_to_transfer);
        stream.memcpyHtoDAsync(d_a_chunk_slice,
                               static_cast<char *>(a_matrix_host) + offset,
                               bytes_to_transfer);
      }
      // get offset for this chunk in a_packed
      cu::DeviceMemory d_a_packed_chunk(
          d_a_matrix_packed,
          pixel_start * samples_padded / CHAR_BIT +
              complex_offset_device_packed,
          local_npixels * samples_padded / CHAR_BIT);
      // run packing kernel
      ccglib::packing::Packing packing(local_npixels * samples_padded, device,
                                       stream);
      packing.Run(d_a_chunk, d_a_packed_chunk, ccglib::packing::pack,
                  ccglib::packing::complex_first);
    }
  }
  ...
  cu::DeviceMemory d_a_matrix_packed_imag(
      d_a_matrix_packed, bytes_a_matrix_packed / 2, bytes_a_matrix_packed / 2);
  conjugate_1bit<<<grid, threads, 0, stream>>>(
      reinterpret_cast<unsigned *>(
          static_cast<CUdeviceptr>(d_a_matrix_packed_imag)),
      bytes_a_matrix_packed / 2);
  ...
  stream.memcpyDtoHAsync(a_matrix_output, d_a_transposed,
